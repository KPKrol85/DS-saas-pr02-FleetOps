...
- **Evidence:** `tests/smoke.spec.js:372-417`, with the two assertions observed failing at `tests/smoke.spec.js:406` and `tests/smoke.spec.js:411`; repeated executions of `npx playwright test -g "renders a precached but unvisited route" --repeat-each=6` performed after the audit on 2026-08-10, outside the audit's own verification pass
- **Current behavior:** "renders a precached but unvisited route styled and interactive on a first offline navigation" does not produce a stable result on repetition. Six consecutive executions on the tree carrying the build-derived cache name failed twice; six consecutive executions on the committed baseline — the previous hand-maintained `CACHE_NAME` literal and the previous plugin, with the test itself unchanged — failed three times. The behaviour therefore precedes the cache-name derivation recorded as the resolved cache-versioning finding and is not caused by it; it is a separate, still-open item. Both observed failures are the same condition rather than two defects: after `context.setOffline(true)` the precached-but-unvisited `/contact/` document renders and its title and heading assert correctly, but the production stylesheet or the production entry script does not take effect — one run failed on the computed `--accent` custom property being empty, another on the FAQ header never receiving `aria-expanded`. The focused service-worker group (`-g "service worker"`, five tests) passed on every execution, and one complete suite run passed 30 of 30, while an earlier complete run failed on this test alone.
- **Impact:** No production defect is evidenced, and no other test in the suite showed instability. The cost falls on verification: `npm run test:smoke` is the project's only automated gate, no CI runs it independently, and `playwright.config.js` deliberately forces a fresh build so that a reported pass describes the current sources — a gate built for exactly that purpose is weakened by a test that can fail for a reason unrelated to the change under test. The realistic hazard is habituation: a suite that fails occasionally for a known-benign reason trains a maintainer to re-run rather than read, which is the state in which a genuine offline regression in this same test would also be dismissed. It is not established here whether the intermittency is a harness timing artifact around the moment the context goes offline or a narrow race in worker-controlled asset serving, and the second possibility is the one that would matter to users.
- **Recommended direction:** Establish which side of that boundary the failure sits on before changing the test — for instance by capturing, on a failing run, whether the hashed asset request was rejected, served from the worker's cache or never issued. If the cause is a missing precondition, make the precondition explicit, such as asserting that the hashed assets are actually retrievable from the cache before the context goes offline. A retry, a longer timeout or a relaxed assertion would remove the signal rather than the defect and would leave the suite unable to detect the regression the test exists to catch.
- **Verification criteria:** The test passes on at least twenty consecutive repeated executions in both focused and full-suite context, and whatever change achieves that is a strengthened precondition or a fixed race rather than a retry, an increased timeout or a weakened assertion.
